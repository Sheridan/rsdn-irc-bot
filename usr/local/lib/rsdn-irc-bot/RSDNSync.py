...
class CRSDNSync(Thread, CConfigurable):
    def __init__(self):
        ...
        self.client = Client("http://www.rsdn.ru/ws/janusAT.asmx?WSDL")
    ...
    def syncForumsList(self):
        self.mutex.acquire()
        print "sync forum list"
        mForumRequest = self.client.factory.create('ForumRequest')
        ...
        result = self.client.service.GetForumList(mForumRequest)
        self.mutex.release()
        groups = dict()
        #print result
    ...
    def syncForumsData(self):
        ...
        #print self.client.service.Check()
        self.mutex.acquire()
        print "sync forum data"
        mChangeRequest = self.client.factory.create('ChangeRequest')
        ...
        for fid in self.forums.keys():
            forum = self.forums[fid]
            mRequestForumInfo = self.client.factory.create('RequestForumInfo')
        ...
        result = self.client.service.GetNewData(mChangeRequest)
        #print result
        self.mutex.release()
    ...
    def getTopic(self, mid):
        self.mutex.acquire()
        mTopicRequest = self.client.factory.create('TopicRequest')
        ...
        result = self.client.service.GetTopicByMessage(mTopicRequest)
        self.mutex.release()
        msgcount = 0
        users = dict()
        message = dict()
        if len(result['Messages']):
            ...
            for m in result['Messages'][0]:
                ...
                users[nick] = 1 if nick not in users else users[nick] + 1
                if m['messageId'] == mid:
                    message['self'] = {
